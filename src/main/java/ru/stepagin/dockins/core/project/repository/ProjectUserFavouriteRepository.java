...
public interface ProjectUserFavouriteRepository extends JpaRepository<ProjectUserFavouriteEntity, Long> {
    ...
    @Query("select count(p) from ProjectUserFavouriteEntity p where p.user.id = :id and p.deleted = false")
    ...
    Optional<ProjectUserFavouriteEntity> findByUserAndProject(@Param("user") AccountEntity user, @Param("project") ProjectInfoEntity project);
